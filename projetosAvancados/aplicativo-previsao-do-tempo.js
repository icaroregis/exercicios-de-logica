...
class Temperatura {
  constructor(dadosDoClima) {
    this.dadosDoClima = dadosDoClima;
  }

  obterLocalizacao() {
    return `${this.dadosDoClima.location.name}, ${this.dadosDoClima.location.country}`;
  }

  obterIcone() {
    return this.dadosDoClima.current.weather_icons[0];
  }

  obterTemperatura() {
    return `${this.dadosDoClima.current.temperature}&deg;C`;
  }
}
